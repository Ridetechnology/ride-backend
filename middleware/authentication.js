  ...
  await verifyUserToken(req, res, () => {
    if (req.user.role === "superadmin") {
      ...
      res.status(403).json({
        ...
        auth: "failed",
        reason:
          "You do not have to permission to execute this task, Super-admin Permission Required",
      });
    }
  });
};
  ...
  await verifyUserToken(req, res, () => {
    if (req.user.role === "subadmin" || "admin" || "superadmin") {
      ...
      res.status(403).json({
        ...
        auth: "failed",
        reason:
          "You do not have to permission to execute this task, sub-admin Permission Required",
      });
    }
  });
};
